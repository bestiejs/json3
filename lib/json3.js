  ...
  if (!has("json")) {
    ...
    if (!has("json-stringify")) {
      ...
      var quote = function (value) {
        var result = '"', index = 0, symbol, isLarge = charIndexBuggy && value.length > 10;
        if (isLarge) {
          // Split the string into an array of characters if the environment
          // doesn't support accessing characters by index. This is only done
          // for longer strings, where the performance gains are significant.
          value = value.split("");
        }
        for (; symbol = value[index]; index++) {
          ...
          switch (symbol) {
            ...
            case "\\": case '"': case "\b": case "\f": case "\n": case "\r": case "\t":
              result += Escapes[symbol];
              break;
            default:
              result += symbol < " " ?
                Escapes[symbol] || (Escapes[symbol] = "\\u00" + toPaddedString(2, symbol.charCodeAt(0).toString(16))) : symbol;
          }
        }
        return result + '"';
      };
