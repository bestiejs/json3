...
(function () {
  ...
  // Serializes a JavaScript `value` as a JSON string. The optional `select`
  // argument may be either a function that alters how certain values in objects
  // and arrays are serialized, or an array of strings and numbers that selects
  // object properties for serialization. The optional `whitespace` argument may
  // be either a string or number that specifies the indentation level of the
  // output.
  Prim.stringify = function (value, select, whitespace) {
    var callback, properties, length, element;
    if (Object(select) === select) {
      if (typeof select == "function") {
        callback = select;
      } else if (getClass.call(select) == "[object Array]") {
        // Convert the property names array into a makeshift set.
        properties = {};
        for (length = select.length; length--;) {
    ...
    if (whitespace != null) {
      if (getClass.call(whitespace) == "[object Number]") {
        whitespace = !whitespace ? "" : Array(whitespace <= 10 ? whitespace : 10).join(" ");
      } else if (getClass.call(whitespace) == "[object String]") {
