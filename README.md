...
The JSON 3 parser does **not** use `eval` or regular expressions. This provides security and performance benefits in obsolete and mobile environments, where the margin is particularly significant. [Benchmarks](http://jsperf.com/crockfords-json-libraries/2) have shown that JSON 3 is slower than the `eval`-based JSON 2 parser, but approximates the speed of Crockford's [recursive descent parser](https://github.com/douglascrockford/JSON-js), Mike Samuel's [`json-sans-eval`](http://code.google.com/p/json-sans-eval/), and Asen Bozhilov's [`json.js`](https://github.com/abozhilov/json).
